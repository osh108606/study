...
public class WeaponInventoryCanvase : MonoBehaviour
{
    ...
    List<UserWeapon> userWeapons = new List<UserWeapon>();
    ...
    public void OpenWeaponInventory(WeaponUseType type ,WeaponSlotType setUpType)
    {
        ...
        userWeapons.Clear();
        for (int i =0;i< User.Instance.userData.userWeapons.Count; i++)
        {
            WeaponItemData data = ItemManager.Instance.GetWeaponItemData(User.Instance.userData.userWeapons[i].key);
            if(data.weaponInfo.weaponUse == type)
            {
                userWeapons.Add(User.Instance.userData.userWeapons[i]);
            }
        }
        UpdateInventory();
    }
    ...
    public void UpdateInventory()
    {
        ...
        for (int i = 0; i < userWeapons.Count; i++)
        {
            ...
            // ???? ?????? ?????? ???? ???????? ???????? UI?? ???? ?????? ??????????.
            panel.SetWeapon(userWeapons[i]);
        }

        
        UserWeapon userWeapon = User.Instance.GetSetUpWeapon(weaponSetUpType);
    ...
    {
        // ???? ?????????? ???? WeaponInfoContainer ?????????? ????, ?????? ???? ?????? ??????????.
        GetComponentInChildren<WeaponInfoContainer>().SetUserWeapon(userWeapon);
    }
}
